...
BIG_MONSTER::BIG_MONSTER(int grid_x, int grid_y, const char* name, Map* map) {
    ...
    this->map = map;
    this->name = name;
    color = al_map_rgb(38, 100, 100);

    sprite = load_bitmap("Assets/big_monster.png");
}

BIG_MONSTER::BIG_MONSTER(const char* name, Map* map) {
    pair<int, int> spawn_grid_pt = generateRandomSpawnGridPoint(map, 3, 3);
    drawrec = rec = get_rec_by_pt_on_board(spawn_grid_pt.first, spawn_grid_pt.second, 3, 3);
    next_facing = facing = NONE;
    next_speed = speed = 1;
    hp = 250.0;
    mp = 100.0;
...
void BIG_MONSTER::draw() {
    ...
    RECTANGLE hp_bar(drawrec.x, drawrec.y - shift, drawrec.w * (float)(fmax(hp, 0.0) / 250), shift / 2);
